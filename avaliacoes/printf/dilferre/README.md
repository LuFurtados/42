...
**Algorithm:**
The implementation follows a **Dispatcher Pattern**:
1.  **Scanning:** The main function `ft_printf` iterates through the format string. Ordinary characters are printed immediately using `ft_putchar_pf`.
2.  **Dispatching:** When a `%` is encountered, the next character (the specifier) is passed to a dispatcher function (`ft_check`).
